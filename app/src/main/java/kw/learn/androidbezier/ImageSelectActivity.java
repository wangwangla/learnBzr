...
import kw.learn.androidbezier.adapter.ImageGridApter;
 ...
 * @Date 2023/4/6 9:50
 */
public class ImageSelectActivity extends Activity implements Callback, OnClickListener,
        OnRecyclerViewClickListener, Observer {
    ...
    private void initData() {
        ImageUtils.queryGalleryPicture(this, getIntent().getStringExtra("data"),
                mHandler, MSG_PHOTO);
    ...
    private void initView() {
        ...
        mAdapter = new ImageGridApter(this,
                ImageSelectObservable.getInstance().getFolderAllImages(),
                mIsSelectSingleImge, getIntent().getIntExtra("maxCount", 9));
        ...
        switch (v.getId()) {
            case R.id.tv_photo_scan:
                if (mAdapter.getSelectlist().size() <= 0) {
                    ...
                    PreviewImageActivity.startPreviewActivity(ImageSelectActivity.this, REQUEST_PREVIEW_PHOTO);
                }
                break;
...
//                mOkTv.setText(getResources().getString(R.string.photo_ok, mAdapter.getSelectlist().size()));
    ...
    public boolean handleMessage(Message msg) {
        switch (msg.what) {
            case MSG_PHOTO:
                ImageSelectObservable.getInstance().addFolderImagesAndClearBefore(
                        (Collection<? extends ImageFolderBean>) msg.obj);
                mAdapter.notifyDataSetChanged();
                break;
        }
        return false;
